...
def main():
    ...
    list_of_sales = dict()
    managers_dict = {
        manager.prefix: manager for manager in base.get_all_managers()}
    ...
    list_of_true_managers = {
        i.short_fullname: i for i in base.get_all_true_managers()}
    for sale in list_of_sales.values():
        if sale.manager:
            sale.manager = list_of_true_managers.get(sale.manager.short_fullname)
    calculated_data = calculate_full_rating(
        [i for i in list_of_sales.values() if i.manager])
